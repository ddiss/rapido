...
dn: cn=/smb,ou=auto.master,ou=automount,ou=admin,${ldap_dc_suffix}
cn: /smb
objectClass: top
objectClass: automount
automountInformation: ldap:ou=auto.smb,ou=automount,ou=admin,${ldap_dc_suffix} --timeout=60 --ghost

dn: ou=auto.smb,ou=automount,ou=admin,${ldap_dc_suffix}
ou: auto.smb
objectClass: top
objectClass: automountMap

dn: cn=share,ou=auto.smb,ou=automount,ou=admin,${ldap_dc_suffix}
cn: share
objectClass: top
objectClass: automount
automountInformation: -fstype=cifs,credentials=${cifs_creds_path} ://${CIFS_SERVER}/${CIFS_SHARE}
EOF
