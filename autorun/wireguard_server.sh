...
[ -n "$WIREGUARD_SRV_PRIVKEY" ]  || _fatal
[ -n "$WIREGUARD_SRV_EP_PORT" ] || _fatal
[ -n "$WIREGUARD_CLI_TUN_ADDR" ]  || _fatal
[ -n "$WIREGUARD_CLI_PUBKEY" ]  || _fatal
ip link add dev wg0 type wireguard || _fatal
ip address add dev wg0 "$WIREGUARD_SRV_TUN_ADDR" || _fatal

cat  > /wg.conf <<EOF
[Interface]
PrivateKey = $WIREGUARD_SRV_PRIVKEY
ListenPort = $WIREGUARD_SRV_EP_PORT

[Peer]
PublicKey = $WIREGUARD_CLI_PUBKEY
AllowedIPs = $WIREGUARD_CLI_TUN_ADDR
EOF
