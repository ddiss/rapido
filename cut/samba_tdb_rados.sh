...
_rt_require_dracut_args "$vm_ceph_conf" "$RAPIDO_DIR/autorun/samba_tdb_rados.sh"
_rt_require_ceph
...
gdb_rpms="python3 python3-base gdb"
gdb_files="$(rpm -ql $gdb_rpms)" || _fail "missing rpm(s) in: $gdb_rpms"
# filter out unneeded pyc & doc files
gdb_files=$(echo "$gdb_files" | grep -v -e "\.pyc$" -e "/doc/" -e "/man/")

"$DRACUT" --install "tail ps rmdir resize dd vim grep find df sha256sum \
		   which touch cut chmod true false seq $gdb_files \
		   getfattr setfattr chacl attr killall sync \
		   id sort uniq date expr tac diff head dirname ss ip ping \
		   ${SAMBA_SRC}/bin/smbpasswd \
		   ${SAMBA_SRC}/bin/smbstatus \
		   ${SAMBA_SRC}/bin/modules/vfs/ceph.so \
		   ${SAMBA_SRC}/bin/smbd \
		   ${SAMBA_SRC}/bin/tdb_radosd \
		   ${SAMBA_SRC}/bin/smbtorture \
		   $LIBS_INSTALL_LIST" \
